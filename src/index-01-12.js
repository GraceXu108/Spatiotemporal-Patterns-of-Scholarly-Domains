...
function createMap() {
    ...
    map.on('style.load', function () {
        ...
        // add STC layer for selected provinces
        addMapLayer(GeoData, selectedProvinces);

    });
}

// Add STC layer for selected provinces
function addMapLayer(geo_data, provinces) {
    map.addLayer({
        id: 'custom_layer',
        type: 'custom',
        renderingMode: '3d',
...
function createBaseMap(geo_data) {
    // It supposed to change base map color according to selectedYear, but too many infos, deleted it. 
    ...
    for (var i = 0; i < geo_data.features.length; i++) {
        ...
        var scale = (Math.log10(totalValues[i]) / Math.log10(Math.max(...totalValues))) * 255;
        ...
        arr.push(Math.round(scale));
        color_arr.push(mathColor);
        ...
        coors.forEach((multipolygon) => {
            ...
            multipolygon.forEach((polygon) => {
                ...
                for (var j = 0; j < polygon.length; j++) {
                    ...
                    shape.lineTo(x, -y);
                    line_vertices.push(new THREE.Vector3(x, -y, 1000));
                }
...
// Draw STCs on the map, according to selected provinces
function createSTCs(geo_data, provinces) {
        ...
        for (var j = 0; j < geo_data.features.length; j++) {
            if (provinces[i] != geo_data.features[j].properties.name) continue;
                ...
                for (var k = 0; k < arr_paper.length; k++) {
                    // console.log(arr_paper[k].json, arr_paper[k].year, geo_data.features[j].properties.center, provinces[i]);
                    create3DTubes(arr_paper[k].json, arr_paper[k].year, geo_data.features[j].properties.center, provinces[i]);
                }
            }
        }
    }
...
function create3DTubes(json, year, center, province) {
    var totalSize = 0;
    ...
    for (var i = 1; i < nodes.length; i++) {
        ...
        if (d.depth != 2) {
            ...
            arcRadius = (Math.sqrt(d.y / 2) + Math.sqrt(d.y / 2 + d.dy / 2 * (1 + (d.size / d.parent.size)))) / 2;
        }
        ...
        if (d.depth != 2) {
            ...
            r = (Math.sqrt(d.y + d.dy / 2 * (1 + (d.size / d.parent.size))) - Math.sqrt(d.y / 2 + d.dy / 2)) / 2;
        }
...
function createSelector(GeoData) {
    ...
    $('#selectProvinceMap').multipleSelect({
        placeholder: 'Select Provinces on Map',
        selectAll: true,
        // single:true,
        multiple: true,
        width: 320,
    ...
    // Set the default value, checkall
    $('#selectProvinceMap').multipleSelect('setSelects', [11]);
...
function popup(obj, x, y) {
    ...
    createBarChart(GeoData, INTERSECTED.data.name, INTERSECTED.data.province);
    ...
    var arc = d3.svg.arc()
        ...
        .outerRadius(function (d) {
            if (d.depth != 2) {
                ...
                return Math.sqrt(d.y / 2 + d.dy / 2 * (1 + (d.size / d.parent.size)));
            }
        });
...
function createBarChart(geo_data, domain, province) {
    ...
    for (var i = 0; i < geo_data.features.length; i++) {
        if (province != geo_data.features[i].properties.name) continue;
            ...
            for (var j = 0; j < paper_arr.length; j++) {
                if (paper_arr[j].year == 2017) valArr[0].valObj = paper_arr[j].json.children;
                if (paper_arr[j].year == 2018) valArr[1].valObj = paper_arr[j].json.children;
                if (paper_arr[j].year == 2019) valArr[2].valObj = paper_arr[j].json.children;
                if (paper_arr[j].year == 2020) valArr[3].valObj = paper_arr[j].json.children;
            }
        }
    }
    ...
    for (var i = 0; i < valArr.length; i++) {
        var valObj = valArr[i].valObj;

        // mark whether the "domain" has been found
        var flag = false;
        for (var j = 0; j < valObj.length; j++) {
            if (valObj[j].name == domain) {
                flag = true;
                sizeArr.push(valObj[j].size)
                barColor = colors[domain];
                continue
            } else if (valObj[j].children) {
                var childObj = valObj[j].children;
                for (var k = 0; k < childObj.length; k++) {
                    if (childObj[k].name == domain) {
                        flag = true;
                        sizeArr.push(childObj[k].size)
                        barColor = colors[valObj[j].name];
                        continue
                    }
                }
            }
            // if it comes to the end and has not found yet
            if (j == valObj.length - 1 && flag == false) sizeArr.push(0);
        }
    }
...
function popupModel(obj, x, y) {
    ...
    // Change bar chart accordingly
    createBarChart(GeoData, INTERSECTED_M.data.name, INTERSECTED_M.data.province);
}
...
function onModelMouseMove(event) {
    ...
    mouse.y = -(event.clientY / renderer.domElement.clientHeight) * 2 + 1 + 1;
